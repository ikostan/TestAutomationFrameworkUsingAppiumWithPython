...
from tests.android_native.calculator_tests.page_locators.calculator_page_locator import CalculatorPageLocator
from tests.android_native.calculator_tests.page_object_models.element.base_element_model import \
	BaseElementModel
from tests.android_native.calculator_tests.page_object_models.license_page_model import LicensePageModel
...
class CalculatorPageModel:
	...
	def open_license(self):
		...
		open_source_license = BaseElementModel(driver=self._driver,
		                                       locator=CalculatorPageLocator.OPEN_SOURCE_LICENSE)
	...
	def screen_formula(self):
		try:
			return BaseElementModel(driver=self.driver,
			                        locator=CalculatorPageLocator.SCREEN_FORMULA)
	...
	def screen_result(self):
		try:
			return BaseElementModel(driver=self.driver,
			                        locator=CalculatorPageLocator.SCREEN_RESULT)
		except TimeoutException:
			return None
	...
	def clr(self):
		return BaseElementModel(driver=self.driver,
		                        locator=CalculatorPageLocator.CLEAR_BTN)

	@property
	def del_btn(self):
		return BaseElementModel(driver=self.driver,
		                        locator=CalculatorPageLocator.DEL_BTN)
